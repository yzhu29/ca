...
uint8_t ghist[HISTBUFFERLENGTH]; // global history
int ptghist; // Just a helper, used to index the current global history bit in ghist[]
...
#ifdef SHARINGTABLES // could have problem
	for (int i = 2; i <= STEP1 - 1; i++)
...
//  bool GetPrediction (UINT32 PC)
  bool GetPrediction (UINT32 PC, bool btbANSF, bool btbATSF, bool btbDYN) // The 3 right parameters are not used
  {
...
//  int Gpredict (UINT32 PC, long long BHIST, int *length, int8_t ** tab, int NBR, int logs)
		// BHIST: Branch History
		// *length: the length of each history that will be used for indexing. e.g. Each Gm entry contains the length of a history.
		// **tab: tab will point to a the "PHT" table of each predictor.
		// NBR: the number of tables of a certan type of GEHL
		// logs: log of the number of entries of each table.
...
//Choser uses TAGE confidence and |LSUM|
	pred_taken = SCPRED;
        if (HighConf) // if TAGE has high confidence
	  {
	    if ((abs (LSUM) <
  ...
  void HistoryUpdate (uint32_t PC, uint16_t brtype, bool taken,
		      ...
		      long long &GBRHIST)
                //(PC, OPTYPE_JMP_DIRECT_COND, resolveDir, branchTarget,  phist,
		       //ptghist, ch_i, ch_t[0],
		       //ch_t[1], L_shist[INDLOCAL],
		       //S_slhist[INDSLOCAL], T_slhist[INDTLOCAL], HSTACK[pthstack], GHIST);
		       // target: Branch Target
		       // X: could be global path history
		       // Y: just a helper, used to index the current global history bit in ghist[]
  {
...
//    if (brtype ==  OPTYPE_RET) {pthstack= (pthstack-1) & 15;
    if (brtype == OPTYPE_RET_COND || brtype == OPTYPE_RET_UNCOND ) {pthstack= (pthstack-1) & 15;
    }

//    if (brtype ==  OPTYPE_CALL_DIRECT){
    if (brtype == OPTYPE_CALL_DIRECT_COND || OPTYPE_CALL_DIRECT_UNCOND){
...
// PREDICTOR UPDATE
  ...
  void UpdatePredictor (UINT64 PC, OpType opType, bool resolveDir, bool predDir,
			UINT64 branchTarget,
			 bool btbANSF, bool btbATSF, bool btbDYN) // The 3 right parameters are not used
  {
...
// Dynamically adjust MGSC threshold
								...
								(32 -
								 ...
								 i)) ^ (bhist
									>> (40
									    ...
									    *
									    i)))
             & ((1 << (logs-(i >=(NBR-2)))) - 1); // This line cause two different sizes for each type of GEHL
